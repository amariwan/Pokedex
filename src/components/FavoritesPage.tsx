		...
		<main className='relative mx-auto w-full max-w-6xl px-4 py-16 sm:px-6 lg:px-10'>
			<section className='rounded-3xl border border-slate-200/60 bg-white/80 p-10 text-center shadow-xl backdrop-blur dark:border-white/10 dark:bg-slate-900/50'>
				<div className='mx-auto flex max-w-2xl flex-col items-center gap-6'>
					<h3 className='text-3xl font-semibold text-slate-900 dark:text-white sm:text-4xl'>Your Favorite Pokémon</h3>
					<p className='text-sm text-slate-600 dark:text-slate-300'>All the creatures you marked as favorites are collected below. Continue exploring to grow your roster.</p>
					<SearchBar searchText={searchText} onSearchChange={(text) => setSearchText(text)} className='w-full' />
				</div>
			</section>
			...
			<section className='mt-12 flex flex-col gap-6'>
				<div className='flex flex-col items-center gap-2 text-center'>
					...
					<p className='text-sm text-slate-600 dark:text-slate-300'>Use the search above to quickly jump to any of your favorites.</p>
				</div>
				<div className='flex flex-wrap justify-center gap-6'>
					{filteredFavorites.length === 0 ? (
						<div className='w-full rounded-3xl border border-dashed border-slate-200/70 bg-white/70 p-12 text-center text-slate-500 backdrop-blur dark:border-white/20 dark:bg-slate-900/40 dark:text-slate-300'>
							<p>No favorites yet. Browse the collection and tap the heart icon to save Pokémon here.</p>
