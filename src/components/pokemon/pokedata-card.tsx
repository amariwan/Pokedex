		...
		<Card className='flex flex-col gap-8  md:flex-col'>
			<CardHeader className='flex flex-col gap-4'>
				<CardTitle className='flex flex-col md:flex-row gap-4 flex-wrap'>
					<span className='scroll-m-20 text-4xl font-extrabold tracking-tight lg:text-5xl'>{capitalize(pokemonData.name)}</span>
					<span className='scroll-m-20 border-b pb-2 text-3xl font-semibold tracking-tight transition-colors first:mt-0'>#{pokemonData.id ?? 0}</span>
				</CardTitle>
				<div>
				...
				<div className='flex flex-col items-center'>
					<Suspense fallback={<div>Loading...</div>}>
						<img className='w-52 h-52 bg-transparent my-4 object-contain' src={pokemonImageURL} alt='pokemon image' />
					</Suspense>
				</div>
			</CardHeader>
			...
			<CardContent>
				...
				<div className='w-full pt-5'>
					{pokemonData.stats.map((stats, index) => (
						<div key={index}>
							<p>
								{capitalize(stats.stat.name)} : <AnimatedValue round={0} value={stats.base_stat} />
							</p>
